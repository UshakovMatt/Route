...
class ViewController: UIViewController {
    ...
    //отслеживает нажатие ндобовления
    ...
    override func viewDidLoad() {
        ...
        // указываем то что карта работает на нашем экране
        mapView.delegate = self
        
        // закрепление элементов на экране
        setConstraints()
        
        // отслеживаем нажати кнопок и присваевам таргет
    ...
    // запрос на прокладывает маршрут
    private func setupPlacemark(addresPlace: String) {
        ...
        geocoder.geocodeAddressString(addresPlace) { [self] (placemarks, error) in
            ...
            if let error = error {
                print(error)
                alertError(title: "Ошибка", massage: "Сервер недоступен. Попробуйте добавить адрес еще раз.")
            }
